...
- `surnameHistory{}` (panneau « histoire d'une graphie ») et `historyCards[]` (encarts historiques : `title`, `intro`, `timeline[]{date,text,strong}`, `paragraphs[]`, `figures[]{file,full,alt,caption}` (ou `figure{…}` au singulier ; plusieurs figures s'empilent en colonne à côté de la frise) — `file` = détail dans `img/`, `full` = version ouverte en lightbox (scan `sources/`, ou dérivé `img/` pour une illustration externe type estampe)) — rendus entre le panneau graphie et les sources.
...
- **`sources/` layout**: files referenced by `genealogie.json` live at the **root** (a `sources[].file` path must never point into a subfolder without updating the JSON). Untracked working material is sorted into subfolders: `originaux-pdf/` (raw AD62/SHD PDF downloads whose extracted JPG/PNG are the referenced scans, renamed with cote+vue), `recherche/` (Filae/Geneanet/recensement screenshots, transcriptions, leads not yet ingested), `historique-110e-ri/` (45-page Argonnaute booklet), `photos-originales/` (uncropped photo originals; portraits shown on-site are `img/` derivatives), `archives-projet/` (first-draft site snapshots). New downloads should be renamed on arrival (kebab-case, contents + date + cote) instead of keeping browser names (`ad62_-N.pdf`, `Screenshot …`).
